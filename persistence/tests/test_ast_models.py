...
def test_ast_uniqueness_and_check_constraints_are_declared() -> None:
    document_names = {constraint.name for constraint in AstDocument.__table__.constraints}
    node_names = {constraint.name for constraint in AstNode.__table__.constraints}
    link_names = {constraint.name for constraint in AstNodeLink.__table__.constraints}

    assert "uq_ast_documents_analysis_source_file" in document_names
    assert "uq_ast_documents_analysis_key" in document_names
    assert "uq_ast_nodes_document_key" in node_names
    assert "uq_ast_nodes_parent_ordinal" in node_names
...
def test_ast_schema_creates_in_sqlite_with_indexes() -> None:
    ...
    assert "ix_ast_nodes_document_parent_ordinal" in node_indexes
    assert "ix_ast_nodes_document_range" in node_indexes
    engine.dispose()
